...
use crate::{deserialize_inputs, InputSignalsInfo};
...
pub fn get_constants(circuit: &Circuit) -> Vec<Node> {
...
pub fn build_circuit_flow(
    ...
    let mut program_archive = match parser_result {
        ...
        Ok((program_archive, warnings)) => {
            if !warnings.is_empty() {
                log::warn!("Parser warnings:");
                for warning in warnings {
                    log::warn!("{}", warning.get_message());
                }
            }
            program_archive
        }
    ...
    let circuit = run_compiler(
        ...
        Config {
            debug_output: false,
            // produce_input_log: true, // TODO: Originally set to true
    ...
    postcard::to_stdvec(&(&nodes.0, &witness_node_idxes, &input_signals)).unwrap()
}
